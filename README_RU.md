...
- 🔐 **Своя страница входа и cookie-сессия** — логин + готовый хеш пароля в ENV, хеш генерируется на странице **Инструменты**; после серии неверных паролей вход блокируется. Плюс CSRF-guard на изменяющих запросах
...
> [!IMPORTANT]
> Панель спрашивает логин и пароль на своей странице входа (не браузерным окном basic auth).
> **По умолчанию `admin` / `admin` — смените пароль.**
> *Инструменты → Хеш пароля*: введите новый пароль, получите md5-хеш, положите его в env
> `WEB_PASSWORD_HASH`. Сам пароль в env не хранится. Пока пароль дефолтный, панель показывает
...
- **Инструменты** — md5crypt-хеш для `WEB_PASSWORD_HASH`; конвертеры AmneziaWG/WireGuard `.conf` (включая **AWG 3.0/3.1**), TrustTunnel `.toml` и OpenVPN `.ovpn` в блок `proxies`; бэкап и восстановление.

**Импорт / экспорт.** Кнопки «скачать» и «загрузить» в заголовке списка файлов работают с открытым файлом: скачивают его на диск и подставляют файл с диска в редактор (сохранение — как обычно, отдельной кнопкой, после проверки). Весь проект целиком — *Инструменты → Бэкап и восстановление*:
...
Хеш по умолчанию — это хеш пароля `admin`, поменяй его через **Инструменты → Хеш пароля**.
...
|---|---|---|
| `WEB_USER` | `admin` | Логин веб-панели. Старое имя `BASIC_AUTH_USER` тоже работает. |
| `WEB_PASSWORD_HASH` | `$1$mihomors$BipEGg3TOdgaQSFfGtisO1` (= хеш `admin`) | **Готовый хеш** пароля: `$1$` (md5crypt, генерируется на странице **Инструменты**), `$5$` или `$6$` (`openssl passwd -5` / `-6`). Старое имя `BASIC_AUTH_HASH` тоже работает. Пустое значение = дефолт, а не «без пароля». |
| `WEB_AUTH` | `on` | `off` — осознанно открыть панель **без пароля**. Единственный способ отключить вход. Старое имя `BASIC_AUTH` тоже работает. |
...
- **Смените дефолтный пароль.** Пока он `admin`, панель показывает предупреждение сверху, а контейнер пишет `WARNING` в лог. Хеш генерируется в *Инструменты → Хеш пароля*, в env уезжает только хеш. Забыли пароль — удалите env `WEB_PASSWORD_HASH` и перезапустите контейнер, вернётся `admin`.
- **Вход и сессии.** Пароль проверяется CGI-скриптом по хешу из env, после входа браузер получает cookie `sid` (`HttpOnly`, `SameSite=Strict`, 7 дней без активности). Сессии живут в ОЗУ контейнера и умирают при его рестарте — тогда нужно войти заново. Кнопка «выйти» в статусной строке убивает сессию. После 5 неверных паролей подряд вход блокируется на 30 секунд, каждая ошибка добавляет паузу до 3 секунд.
- **Панель работает по HTTP без TLS**: пароль при входе и в генераторе хеша идёт по локальной сети в открытом виде. Не выставляйте порт `80` контейнера в интернет — только LAN или VPN.
- **CSRF-guard.** Cookie с `SameSite=Strict` не отправляется по запросам с чужих сайтов, а все изменяющие эндпоинты дополнительно принимают только `POST` и только со своим `Referer`. Второй слой отключается через `WEB_CSRF=off`.
...
Если панель нужна из скриптов (curl/автоматизация): сначала войдите и сохраните cookie, дальше отправляйте её с каждым запросом. `POST`-only при этом остаётся; с `WEB_CSRF=off` заголовок `Referer` не нужен.
...
/www/                            веб-панель (index.html + login.html + assets + cgi-bin)
...
> ⚠ **armv5: веб-панель не работает.** busybox httpd в текущем `rootfs.tar` собран без CGI (и без basic auth): `/cgi-bin/*` отдаётся как текст, `POST` отвечает 501. Ядро mihomo при этом запускается. Починится только пересборкой rootfs с `CONFIG_FEATURE_HTTPD_CGI=y`.

**gVisor.** Обычные теги содержат ядро **без gVisor** — оно на ~9 МиБ меньше, а дефолтному конфигу (`tun.stack: system`) gVisor не нужен. Если используешь `tun.stack: gvisor` / `mixed` или исходящий Tailscale — бери тот же тег с суффиксом `-gvisor`: `latest-gvisor`, `amd64v1-gvisor`, `amd64v2-gvisor`, `<версия>-gvisor`.
...
Локальная сборка по умолчанию берёт ядро upstream MetaCubeX (`MIHOMO_CUSTOM_CORE=0`), а оно всегда с gVisor; деление на обычный/`-gvisor` относится к ядру `mihomo-proxy-ros` (`MIHOMO_CUSTOM_CORE=1`, так собирает CI).

| `AMD64VERSION` | Инструкции | CPU | Тег образа |
|---|---|---|---|
| `v1` | baseline SSE2 | любой x86-64 | `amd64v1` / `amd64v1-gvisor` |
| `v2` | SSE3/SSSE3/SSE4.x | ~2009+ | `amd64v2` / `amd64v2-gvisor` |
| `v3` (по умолч.) | AVX/AVX2/BMI | ~2015+, быстрее | `latest` / `latest-gvisor` |
